...
export const AuthProvider = ({ children }) => {
  ...
  const currentRoleInfo = ROLE_INFO[activeRole] || ROLE_INFO[ROLES.CASHIER]
  const visibleModules = user ? getVisibleModules(activeRole) : []
  const landingPath = ROLE_LANDING_PATH[activeRole] || '/dashboard/orders'

  /**
   * এই role টা module টা "দেখতে" পারবে কিনা।
   * প্রথমে role matrix, তারপর assigned permission group এর override.
   */
  const canAccess = (moduleId) => {
    ...
    // Permission group শুধু আরো সংকুচিত করতে পারে, কখনো প্রসারিত করতে পারে না
    const group = permissionGroups.find((g) => g.id === profile?.permission_group_id)
    const groupRule = group?.permissions?.[moduleId]
    if (groupRule && groupRule.view === false) return false

    return true
  }

  /** এই role টা module এ লেখা/পরিবর্তন করতে পারবে কিনা। */

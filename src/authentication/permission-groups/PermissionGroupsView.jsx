...
import { ROLES, ROLE_INFO } from '../constants/rbac'
...
export const PermissionGroupsView = () => {
  ...
  // এই group এ থাকা user দের profiles.role কী হবে
  const [baseRole, setBaseRole] = useState(ROLES.CASHIER)
  ...
  const handleCreateGroupSubmit = (e) => {
    ...
    try {
      createPermissionGroup({
        ...
        // base role — এই group এ যাকে রাখা হবে, তার profiles.role এটাই হবে
        role: baseRole,
        ...
        <div style={styles.modalOverlay} onClick={() => setIsModalOpen(false)}>
          <div style={styles.modalCard} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreateGroupSubmit} style={styles.form}>
              ...
              {/* Base system role — এটাই profiles.role এ সেভ হবে */}
              <div style={styles.formGroup}>
                <label style={styles.label}>Base System Role *</label>
                <select
                  value={baseRole}
                ...
                >
                  {Object.values(ROLES).map((r) => (
                    <option key={r} value={r}>
                      {ROLE_INFO[r].name} — {ROLE_INFO[r].title}
                    </option>
                  ))}
                </select>
                <p style={styles.fieldHint}>
                  Assigning a staff member to this group writes exactly this value to
                  the <code>profiles.role</code> column in Supabase. The toggles below
                  can only narrow what this role may do — never widen it.
                </p>
              </div>
